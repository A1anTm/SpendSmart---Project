  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-gradient-to-br from-blue-50 to-indigo-100">
      <div className="w-full md:flex-1 flex flex-col justify-center px-6 md:px-12 py-8">
        ...
        <h2 className="text-3xl md:text-4xl font-bold text-gray-800 mb-8 md:mb-12 leading-tight">
      ...
      <div className="w-full md:w-[540px] flex flex-col justify-center px-6 md:px-8 py-8">
        {mode === 'forgot-password' ? (
          ...
          <div className="bg-white rounded-2xl shadow-xl p-6 md:p-8 w-full max-w-md mx-auto">
            <h2 className="text-2xl font-bold text-gray-800 mb-2 text-center">Bienvenido</h2>
            <p className="text-gray-500 text-sm mb-6 text-center">Accede a tu cuenta o crea una nueva para comenzar</p>
            ...
            {mode === 'login' ? (
              <>
                ...
                <div className="mt-4 text-center">
                  <button onClick={handleForgotPassword} className="text-teal-600 hover:text-teal-700 text-sm">¿Olvidaste tu contraseña?</button>
                </div>
              </>
            ) : (
...
function TabButton({ active, children, onClick }: { active: boolean; children: React.ReactNode; onClick: () => void; }) {
  return (
    <button onClick={onClick} className={`flex-1 py-2 px-3 text-sm font-medium rounded-md transition-colors ${active ? 'bg-white  text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
      {children}
    </button>
  );
}
